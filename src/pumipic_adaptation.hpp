...
// LOCAL COPY added to Comet's pumi-pic for the Comet AMR integration (beta b.2).
// Source: SCOREC/pumi-pic branch ac/coarsen-adaptation @ afd972691f583ed94ea74fefac1e16b5a0a79a4e
// Copied onto pumi-pic 88b803e (the version Comet builds against), which has no
// common ancestor with the adaptation branch; the particle-structure public interface
// (get<>, Slice(pid,i), nPtcls, capacity, rebuild, ps::parallel_for) is identical, so
// only the edits below were needed. Port edits vs the upstream afd9726 header:
//   1. Added <Omega_h_adapt.hpp> (UserTransfer base) and <Omega_h_for.hpp> (parallel_for)
//      so the header is self-contained.
//   2. Added KOKKOS_INLINE_FUNCTION to functions called inside device lambdas that
//      lacked it (ModifiedElem ctors, get_indices, update2LowestParent) to silence
//      CUDA host/device warnings.
// No algorithm logic was changed. Long-term: SCOREC to mainline the API (coordination
// item); until then this local copy tracks afd9726.
...
namespace Omega_h {
  ...
  template <Int dim>
  KOKKOS_INLINE_FUNCTION
  Kokkos::Array<Int, dim+1> get_indices(Int index, Int rotation = 0) const {
